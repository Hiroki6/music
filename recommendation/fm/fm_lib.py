...
class CyFmSgdOpt():
    ...
    def smoothing(self):
        ...
        for target_song, tags in self.not_learned_song_tag_map:
            # スムージング対象の楽曲のインデックス
            target_song_index = self.tag_map("song="+str(target_song))
            for learn_song, tags in self.learned_song_tag_map:
                # 学習された楽曲のインデックス
                learn_song_index = self.tag_map("song="+str(learn_song))
